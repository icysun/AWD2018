...
function write_access_log_probably() { 
	...
	$ffff = fopen('/tmp/log/all_requests.txt', 'a'); //日志路径 
    fwrite($ffff, $raw);  
    fclose($ffff);
}
...
function write_access_logs_detailed(){
    ...
    $ffff = fopen('/tmp/log/all_requests_detail.txt', 'a'); //日志路径 
...
function filter_attack_keyword($str){
    if(preg_match("/select\b|insert\b|update\b|drop\b|and\b|or\b|delete\b|dumpfile\b|outfile\b|load_file|rename\b|floor\(|extractvalue|updatexml|name_const|multipoint\(/i", $str)){
    ...
    if(preg_match("/base64_decode|or|eval\(|assert\(|file_put_contents|`|fwrite|curl|system|passthru|exec|system|chroot|scandir|chgrp|chown|shell_exec|proc_open|proc_get_status|popen|ini_alter|ini_restorei/i", $str)){
...
function write_attack_log($alert){
    ...
    $ffff = fopen('/tmp/log/attack_detected_log.txt', 'a'); //日志路径 
    fwrite($ffff, $data);  
    fclose($ffff);
    if($alert == 'GETFLAG'){
        echo "Bye--Bye--"; //如果请求带有flag关键字，显示假的flag。（2333333）
    }else{
        sleep(3); //拦截前延时3秒
    }
    exit(0);
